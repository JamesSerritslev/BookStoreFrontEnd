...
export const handlers = [
  ...
  http.post("http://localhost:8080/api/v1/book", async ({ request }) => {
    ...
    const book = {
      bookId:
        books.length > 0 ? Math.max(...books.map((b) => b.bookId)) + 1 : 1,
  ...
  http.put(
    ...
    async ({ params, request }) => {
      ...
      books[index] = {
        ...
        bookDescription:
          updated.bookDescription ?? books[index].bookDescription,
  ...
  http.post(
    "http://localhost:8080/api/v1/cart/me/items",
    async ({ request }) => {
      const authHeader = request.headers.get("Authorization");
      const token = authHeader?.replace("Bearer ", "");
      ...
      const existingItem = cart.items.find(
        (item: any) => item.inventoryId === inventoryId
      );
      ...
      if (existingItem) {
        ...
        // Extract book ID from inventoryId (format: 00000000-0000-0000-0000-000000000001)
        const bookIdStr = inventoryId.split("-").pop() || "0";
        ...
        // Use the actual book price (convert to cents)
        const unitPrice = Math.round(book.bookPrice * 100);
        const newItem = {
          ...
          // Include book details for display
      ...
      cart.subtotal = cart.items.reduce(
        (sum: number, item: any) => sum + item.lineSubtotal,
        0
      );
  ...
  http.patch(
    "http://localhost:8080/api/v1/cart/me/items/:itemId",
  ...
  http.put("/api/users/:id", async ({ params, request }) => {
    ...
    users[index] = {
      ...
      firstName: updated.firstName ?? users[index].firstName,
      lastName: updated.lastName ?? users[index].lastName,
